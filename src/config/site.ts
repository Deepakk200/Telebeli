...
/** General contact (nav "Contact"). Kept distinct from the demo-request CTA. */
export const contactHref = `mailto:${siteConfig.email}`;

/**
 * Primary nav — clean, conversion-focused, no dropdowns. Every item maps to a
 * real in-page section anchor (Home → top, Contact → mailto); the "Book a Demo"
 * CTA lives in the header. The Security route was removed, so it is gone here.
 */
export const mainNav = [
  ...
  { title: "FAQ", href: "/#faq" },
  { title: "Contact", href: contactHref },
] as const;
