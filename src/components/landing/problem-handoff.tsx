...
export function ProblemHandoff() {
  return (
    <div className="mt-2 rounded-widget bg-white p-2.5 shadow-sm ring-1 ring-pc-hairline">
      <p className="text-[0.75rem] font-semibold tracking-[0.01em] text-ink-600">Warm Handoff</p>

      <div className="mt-2 flex items-center justify-between gap-1.5">
        <span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-lavender-tint text-solution ring-1 ring-indigo-600/20">
        ...
        <div className="flex min-w-0 shrink-0 flex-col gap-0.5 rounded-lg bg-surface-2 p-1 ring-1 ring-pc-hairline">
          {CONTEXT_CHIPS.map((chip) => (
            <span
              key={chip}
              className="whitespace-nowrap rounded-md bg-lavender-tint px-2 py-0.5 text-center text-[0.625rem] font-medium text-solution-strong ring-1 ring-indigo-600/20"
            >
              {chip}
            </span>
          ))}
        ...
        <span className="size-11 shrink-0 overflow-hidden rounded-full shadow-sm ring-2 ring-white">
          <AgentFace className="size-full" />
          <span className="sr-only">Human agent</span>
        </span>
      </div>

      <p className="mt-1.5 flex items-center gap-2 rounded-lg bg-success/10 px-3.5 py-1 text-small font-semibold text-success">
